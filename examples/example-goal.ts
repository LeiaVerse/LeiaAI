// This is a simple example of how to use the Leia package
...
async function fetchContext() {
  try {
    const response = await fetch(
      "https://raw.githubusercontent.com/leiaverse/sleeves/main/sleeves/eternum.json",
    );
...
async function main() {
  ...
  const leia = new ChainOfThought(llmClient, {
    worldState: context,
  });
  ...
  leia.on("action:error", ({ action, error }) => {
    console.log("\n❌ Action failed:", {
      type: action.type,
      error,
    });
  ...
  leia.on("goal:updated", ({ id, status }) => {
  ...
  leia.on("goal:completed", ({ id, result }) => {
  ...
  leia.on("memory:experience_stored", ({ experience }) => {
  ...
  leia.on("memory:knowledge_stored", ({ document }) => {
  ...
  leia.on("memory:experience_retrieved", ({ experiences }) => {
  ...
  leia.on("memory:knowledge_retrieved", ({ documents }) => {
  ...
  while (true) {
    ...
    try {
      ...
      await leia.planStrategy(userInput);
      ...
      while (true) {
        const readyGoals = leia.goalManager.getReadyGoals();
        const activeGoals = leia.goalManager
          .getGoalsByHorizon("short")
          .filter((g) => g.status === "active");
        const pendingGoals = leia.goalManager
          .getGoalsByHorizon("short")
        ...
        if (readyGoals.length === 0 && activeGoals.length === 0) {
          ...
          pendingGoals.forEach((goal) => {
            const blockingGoals = leia.goalManager.getBlockingGoals(goal.id);
      ...
      const recentExperiences = await leia.memory.getRecentEpisodes(5);
      console.log(chalk.blue("\n🔄 Recent Experiences:"));
      ...
      const relevantDocs = await leia.memory.findSimilarDocuments(
        userInput,
        3,
      );
